...
_ref=b("."+(type.replace(' ','.')));for(_i=0,_len=_ref.length;_i<_len;_i++){elem=_ref[_i];if($(elem).data('pos')===coords){return $(elem);}}};movePiece=function(from,to){var capture,square;capture=pieceAt(to);square=b("."+to);return pieceAt(from).css({'z-index':5}).animate({top:square.position().top,left:square.position().left},function(){b("."+from).data('contents',null);b("."+to).data('contents',$(this).data('type'));if(capture!=null){capture.remove();}
return $(this).css({'z-index':1}).data('oldPos',from).data('pos',to);});};enemyAt_=function(coords,col,ignore,add){var piece;if(coords===add){return true;}
if(coords===ignore){return null;}
piece=pieceAt(coords);if(!(piece!=null)){return false;}
return piece.data('type').split(' ')[0]!==col;};allyAt_=function(coords,col,ignore,add){var piece;if(coords===add){return true;}
if(coords===ignore){return null;}
piece=pieceAt(coords);if(!(piece!=null)){return false;}
return piece.data('type').split(' ')[0]===col;};activatePieces=function(){return b(".piece").draggable({disabled:true,revert:'invalid',start:function(ev,ui){var candidate,candidates,_i,_len;candidates=candidateMoves($(this).data('type'),$(this).data('pos'));b("."+($(this).data('pos'))).toggleClass('active');for(_i=0,_len=candidates.length;_i<_len;_i++){candidate=candidates[_i];b("."+candidate).toggleClass('candidate').droppable('option','disabled',false);}
return $(this).data('oldPos',$(this).data('pos')).data('candidates',candidates).css({'z-index':5});},stop:function(ev,ui){var candidate,_i,_len,_ref;b("."+($(this).data('oldPos'))).toggleClass('active').data('contents',null);b("."+($(this).data('pos'))).data('contents',$(this).data('type'));_ref=$(this).data('candidates');for(_i=0,_len=_ref.length;_i<_len;_i++){candidate=_ref[_i];b("."+candidate).toggleClass('candidate').droppable('option','disabled',true);}
return $(this).css({'z-index':1});}});};pieceMoves={bishop:function(x,y,col,threatCheck,ignore,add){var i,returnable;returnable=[];i=1;while(validSquare_(x+i,y+i)&&!(pieceAt(toFAN(x+i,y+i),ignore,add)!=null)){returnable.push(toFAN(x+i,y+i));i++;}
if(validSquare_(x+i,y+i)&&enemyAt_(toFAN(x+i,y+i),col,ignore,add)){returnable.push(toFAN(x+i,y+i));}
i=1;while(validSquare_(x+i,y-i)&&!(pieceAt(toFAN(x+i,y-i),ignore,add)!=null)){returnable.push(toFAN(x+i,y-i));i++;}
if(validSquare_(x+i,y-i)&&enemyAt_(toFAN(x+i,y-i),col,ignore,add)){returnable.push(toFAN(x+i,y-i));}
i=1;while(validSquare_(x-i,y-i)&&!(pieceAt(toFAN(x-i,y-i),ignore,add)!=null)){returnable.push(toFAN(x-i,y-i));i++;}
if(validSquare_(x-i,y-i)&&enemyAt_(toFAN(x-i,y-i),col,ignore,add)){returnable.push(toFAN(x-i,y-i));}
i=1;while(validSquare_(x-i,y+i)&&!(pieceAt(toFAN(x-i,y+i),ignore,add)!=null)){returnable.push(toFAN(x-i,y+i));i++;}
if(validSquare_(x-i,y+i)&&enemyAt_(toFAN(x-i,y+i),col,ignore,add)){returnable.push(toFAN(x-i,y+i));}
return returnable;},knight:function(x,y,col,threatCheck,ignore,add){var returnable;returnable=[];if(validSquare_(x+1,y+2)&&!allyAt_(toFAN(x+1,y+2),col,ignore,add)){returnable.push(toFAN(x+1,y+2));}
if(validSquare_(x+2,y+1)&&!allyAt_(toFAN(x+2,y+1),col,ignore,add)){returnable.push(toFAN(x+2,y+1));}
if(validSquare_(x+2,y-1)&&!allyAt_(toFAN(x+2,y-1),col,ignore,add)){returnable.push(toFAN(x+2,y-1));}
if(validSquare_(x+1,y-2)&&!allyAt_(toFAN(x+1,y-2),col,ignore,add)){returnable.push(toFAN(x+1,y-2));}
if(validSquare_(x-1,y-2)&&!allyAt_(toFAN(x-1,y-2),col,ignore,add)){returnable.push(toFAN(x-1,y-2));}
if(validSquare_(x-2,y-1)&&!allyAt_(toFAN(x-2,y-1),col,ignore,add)){returnable.push(toFAN(x-2,y-1));}
if(validSquare_(x-2,y+1)&&!allyAt_(toFAN(x-2,y+1),col,ignore,add)){returnable.push(toFAN(x-2,y+1));}
if(validSquare_(x-1,y+2)&&!allyAt_(toFAN(x-1,y+2),col,ignore,add)){returnable.push(toFAN(x-1,y+2));}
return returnable;},queen:function(x,y,col,threatCheck,ignore,add){return this.bishop(x,y,col,threatCheck,ignore,add).concat(this.rook(x,y,col,threatCheck,ignore,add));},king:function(x,y,col,threatCheck,ignore,add){var opCol,returnable;returnable=[];if(validSquare_(x,y+1)&&!allyAt_(toFAN(x,y+1),col,ignore,add)){returnable.push(toFAN(x,y+1));}
...
if(validSquare_(x+1,y-1)&&!allyAt_(toFAN(x+1,y-1),col,ignore,add)){returnable.push(toFAN(x+1,y-1));}
if(validSquare_(x,y-1)&&!allyAt_(toFAN(x,y-1),col,ignore,add)){returnable.push(toFAN(x,y-1));}
if(validSquare_(x-1,y-1)&&!allyAt_(toFAN(x-1,y-1),col,ignore,add)){returnable.push(toFAN(x-1,y-1));}
if(validSquare_(x-1,y)&&!allyAt_(toFAN(x-1,y),col,ignore,add)){returnable.push(toFAN(x-1,y));}
if(validSquare_(x-1,y+1)&&!allyAt_(toFAN(x-1,y+1),col,ignore,add)){returnable.push(toFAN(x-1,y+1));}
...
return returnable;},rook:function(x,y,col,ignore,add){var i,returnable;returnable=[];i=1;while(validSquare_(x,y+i)&&!(pieceAt(toFAN(x,y+i),ignore,add)!=null)){returnable.push(toFAN(x,y+i));i++;}
if(validSquare_(x,y+i)&&enemyAt_(toFAN(x,y+i),col,ignore,add)){returnable.push(toFAN(x,y+i));}
i=1;while(validSquare_(x+i,y)&&!(pieceAt(toFAN(x+i,y),ignore,add)!=null)){returnable.push(toFAN(x+i,y));i++;}
if(validSquare_(x+i,y)&&enemyAt_(toFAN(x+i,y),col,ignore,add)){returnable.push(toFAN(x+i,y));}
i=1;while(validSquare_(x,y-i)&&!(pieceAt(toFAN(x,y-i),ignore,add)!=null)){returnable.push(toFAN(x,y-i));i++;}
if(validSquare_(x,y-i)&&enemyAt_(toFAN(x,y-i),col,ignore,add)){returnable.push(toFAN(x,y-i));}
i=1;while(validSquare_(x-i,y)&&!(pieceAt(toFAN(x-i,y),ignore,add)!=null)){returnable.push(toFAN(x-i,y));i++;}
if(validSquare_(x-i,y)&&enemyAt_(toFAN(x-i,y),col,ignore,add)){returnable.push(toFAN(x-i,y));}
return returnable;}};candidateMoves=function(type,fan,threatCheck,ignore,add){var candidates,move,opCol,typeArray,verified,x,y,_i,_j,_len,_len2,_ref;verified=[];_ref=toCart(fan),x=_ref[0],y=_ref[1];typeArray=type.split(' ');candidates=pieceMoves[typeArray[1]](x,y,typeArray[0],threatCheck,ignore,add);if(threatCheck){return candidates;}
opCol=typeArray[0]==='white'?'black':'white';if(typeArray[1]==='king'){for(_i=0,_len=candidates.length;_i<_len;_i++){move=candidates[_i];if(!threatens_(move,opCol)){verified.push(move);}}
return verified;}
for(_j=0,_len2=candidates.length;_j<_len2;_j++){move=candidates[_j];if(!threatens_(b(".king."+typeArray[0]).data('pos'),opCol,fan,move)){verified.push(move);}}
return verified;};threatens_=function(square,color,ignore,add){var piece,_i,_len,_ref;_ref=b(".piece."+color);for(_i=0,_len=_ref.length;_i<_len;_i++){piece=_ref[_i];if($(piece).data('pos')!==add){if(__indexOf.call(candidateMoves($(piece).data('type'),$(piece).data('pos'),true,ignore,add),square)>=0){return true;}}}
return false;};handler={reset:function(){b('.piece').remove();return b('.square').data('contents',null);},getFEN:function(){var counter,file,rank,returnable,typeArray,_i,_len,_ref;returnable='';for(rank=8;rank>=1;rank--){counter=0;_ref='abcdefgh';for(_i=0,_len=_ref.length;_i<_len;_i++){file=_ref[_i];if(b("."+file+rank).data('contents')){typeArray=b("."+file+rank).data('contents').split(' ');if(counter>0){returnable+=counter;}
counter=0;if(typeArray[0]==='white'){returnable+=symDict[typeArray[1]];}else{returnable+=symDict[typeArray[1]].toLowerCase();}}else{counter++;}}
if(counter>0){returnable+=counter;}
...
enPassant=ep;if(typeArray[1]==='king'){castle[typeArray[0]]['short']=false;castle[typeArray[0]]['long']=false;if(ui.draggable.data('oldPos')[0]==='e'){rank=$(this).data('pos')[1];if($(this).data('pos')[0]==='g'){movePiece('h'+rank,'f'+rank);}
if($(this).data('pos')[0]==='c'){movePiece('a'+rank,'d'+rank);}}}
